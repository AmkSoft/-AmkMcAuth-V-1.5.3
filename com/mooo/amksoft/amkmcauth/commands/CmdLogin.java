...
public class CmdLogin implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender cs, Command cmd, String label, String[] args)
    	{
    	// cmd.getname() toont de geregistreerde commando-naam
    	// label bevat het ingetypte commando, kan dus OOK een Alias zijn.
    ...
    private static boolean LogMeIn(Player cs, String cmd, String rawPassword) {
    	if (cmd.equalsIgnoreCase("login")) {
            ...
            boolean MenuPswd=false; // Marker on using Old UserId in front of PassWord
            boolean BCryptOk=false;
            if(ap.getHashType().toLowerCase().contains("bcrypt")) {
            	// We Use BCRYPT Encryption, check this...            	
            	BCryptOk = BCrypt.checkpw(rawPassword, SavedPassword);
            	if(!BCryptOk) {// Regular Password wrong, Check menu variant (Usernamer+Password)
            		BCryptOk = BCrypt.checkpw(ap.getUserName() + rawPassword, SavedPassword);
            		if(BCryptOk) MenuPswd=true;
            	}
            }

            if (hashedPassword.equals(SavedPassword) || hashedMenuPassword.equals(SavedPassword) || BCryptOk) {
        		if(hashedMenuPassword.equals(SavedPassword)) MenuPswd=true;

                ap.login();
                
                // MenuPassword is stored: ReStore NormalPassWord (no UserName in front)
                // So we can fix this later on when player is changing his password.
                if(MenuPswd) ap.setHashedPassword(hashedPassword, ap.getHashType());  
