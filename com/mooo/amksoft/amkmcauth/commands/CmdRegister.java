...
public class CmdRegister implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender cs, Command cmd, String label, String[] args)
    {
        ...
        else rawPassword = AmkAUtils.getFinalArg(args, 0).trim(); // Pass spaces (= Auto Confirm)
    ...
    private static boolean Register(Player cs, String cmd, String rawPassword) {
        if (cmd.equalsIgnoreCase("register")) {
            ...
            // Auto-Password confirm. If players user space in password is it checked as confirmation part 1 and part 2 
            if(rawPassword.contains(" ")) {            	
           		//String myArray[] = new String[2];        		
               	String[] passwords = new String[2];
           		passwords = rawPassword.split(" ") ;
               	if(!passwords[0].contentEquals(passwords[1])) {
            		cs.sendMessage(ChatColor.RED + AmkAUtils.colorize(Language.PASSWORDS_SHOULD_EQUAL.toString()));
            		return true;
               	}
               	rawPassword = passwords[0];
            }
