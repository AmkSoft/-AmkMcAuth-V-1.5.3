...
public class CmdChngPwd implements CommandExecutor {
    ...
    {    	
    	...
    	// On Change Password , as separator goes first
    	if(args.contains(","))
    ...
    private static boolean ChgMyPswd(Player cs, String cmd, String oldRawPassword, String newRawPassword) {
        if (cmd.equalsIgnoreCase("changepassword")) {
            ...
            String newEncPassword; // newPassword is now (15-12-2020) default Username + Password !  
            ...
            try {
            	...
            	newEncPassword = Hasher.encrypt(newRawPassword, Config.passwordHashType); // NoUserName !!
            ...
            if(oldRawPassword.equals("menu")) { // Using pictograms for Password selection
                ...
                if(ap.getHashType().toLowerCase().contains("bcrypt")) {
                	...
                	BCryptOk = BCrypt.checkpw(oldRawPassword, ap.getPasswordHash());
                	// This Extra Check should be obsolete as the password is stored
                	//  without username as is password is (re)stored on LogIn 
                	if(!BCryptOk) // Regular Password wrong, Check menu variant (Username+Password)
                		BCryptOk = BCrypt.checkpw(ap.getUserName() + oldRawPassword, ap.getPasswordHash());
                }
            	
                if (!ap.getPasswordHash().equals(oldMnuPassword) & !ap.getPasswordHash().equals(oldNrmPassword) & !BCryptOk ) {
            ...
            ap.setHashedPassword(newEncPassword, Config.passwordHashType);
