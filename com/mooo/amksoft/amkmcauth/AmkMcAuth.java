...
public class AmkMcAuth extends JavaPlugin {
		...
		for (String fileName : userdataFolder.list(new PatternFilenameFilter("(?i)^.+\\.yml$"))) {
			...
			try {
				...
				while (in.hasNext()) { // 1 more character?: iterates each line in the file
					...
					if(line.contains("  emladdress:")) EmAddress    = line.substring(line.lastIndexOf(" ")+1);
					if(line.contains("  recovertxt:")) RecovrTxt    = line.substring(line.lastIndexOf(" ")+1);
			...
			if(!ImportError) {
				...
				try {
					...
					if (rs.next() == true) { 
						RecAanwezig=rs.getInt("PlayerUUID");
					}
	    	        ps.close();					
					rs.close();
					if(RecAanwezig==0) {
						ps = MySQL.getConnection().prepareStatement(
								...
								"       ( Name, UUID, Joyn, Quit, Login, LoggedIn, Password, Hash, IpAdress, EmlAdress, RecovrTxt, Vip, GodModeEx) " +
								"VALUES (  ?  ,  ?  ,  ? ,   ? ,   ?  ,     ?   ,     ?   ,   ? ,     ?    ,     ?    ,      ?   ,  ? ,     ?    ) " 
								);
						...
						ps.setString(  11,  RecovrTxt);
						ps.setBoolean( 12,  VipPlayer);
						ps.setLong(    13,  Expire);
						ps.executeUpdate();							
			            ps.close();
					}
    ...
    public void onEnable() {
        ...
        if (!Config.MySqlDbHost.equals("")) {
            //if (Config.DatabaseType.equalsIgnoreCase("MongoDB"))      	
            	//MongoDB.connect(); // Connect to the Database System
            //else
            	MySQL.connect(); // Connect to the Database System
        }
        ...
        this.registerCommand(new CmdSetRecvr(this), "setrecover", this);
        this.registerCommand(new CmdRecover(this), "recoverpwd", this);
        this.registerCommand(new CmdChngPwd(this), "changepassword", this);

        AuthListener.setAlias("login", AmkAUtils.getAliasesFor("login"));
        AuthListener.setAlias("logout", AmkAUtils.getAliasesFor("logout"));
        AuthListener.setAlias("register", AmkAUtils.getAliasesFor("register"));
        AuthListener.setAlias("setemail", AmkAUtils.getAliasesFor("setemail"));
        AuthListener.setAlias("setrecover", AmkAUtils.getAliasesFor("setrecover"));
        AuthListener.setAlias("recoverpwd", AmkAUtils.getAliasesFor("recoverpwd"));
        AuthListener.setAlias("changepassword", AmkAUtils.getAliasesFor("changepassword"));
        ...
        PConfManager.countPlayersFromIpAndGetVipPlayers(); // Set PlayerCount variabele
    	...
    	if (getConfig().getBoolean("general.metrics_enabled"))
    	{
            ...
            metrics.addCustomChart(new Metrics.SimplePie("Appl_Usage", new Callable<String>() {
            	@Override
            	public String call() throws Exception {
               	String Usage="ls+"; // Large-Server
            	if(Config.registrationType.equals("password"))
            		Usage = Usage + "Register-Pswrd";

        		if(Config.checkOldUserdata)
        			Usage = Usage + "CheckOldUserData";
    ...
    public void onDisable() {
        ...
        for (Player p : this.getServer().getOnlinePlayers()) {
            ...
            if (Config.useHideInventory) ap.RestoreSurvivalInventory(p); // Restore Inventory
        }
